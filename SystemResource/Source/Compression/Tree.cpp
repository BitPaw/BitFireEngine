...
void gen_codes(ct_data* tree, int max_code, unsigned short* bl_count)
{
    ...
    for (n = 0; n <= max_code; n++)
    {
        ...
        Tracecv(tree != static_ltree, (stderr, "\nn %3d %c l %2d c %4x (%File) ",
            n, (isgraph(n) ? n : ' '), len, tree[n].fc.code, next_code[len] - 1));
    }
}

int  inflate_table(HuffCodeType type, unsigned short* lens, unsigned int codes, HuffCode** table, unsigned  int* bits, unsigned short* work)
{
    ...
    HuffCode here;                  /* table entry for duplication */
    HuffCode* next;             /* next available space in table */
    ...
    if (left > 0 && (type == HuffCodeType::CODES || max != 1))
    ...
    switch (type)
    {
        case HuffCodeType::CODES:
            base = extra = work;    /* dummy value--not used */
            match = 20;
            break;
        case HuffCodeType::LENS:
            base = lbase;
            extra = lext;
            match = 257;
            break;
    ...
    if ((type == HuffCodeType::LENS && used > ENOUGH_LENS) ||
        (type == HuffCodeType::DISTS && used > ENOUGH_DISTS))
        return 1;
